...
#include <filesystem>
#include <cmath>
#include <chrono>  // Zeitmessung (siehe https://en.cppreference.com/w/cpp/chrono)
#include <omp.h>
...
void increase_window_size(const std::vector<unsigned char> &image, std::vector<unsigned char> *temp_window,
                          int old_winsize, const int width, const int height, const int xpos, const int ypos) {

    int half_win = old_winsize / 2;

    for (int x = xpos - half_win - 1; x <= xpos + half_win + 1; x++) {
        if (ypos - half_win - 1 >= 0 && x >= 0 && x <= width - 1) {
...
void median_filter(std::vector<unsigned char> *image, const int width, const int height,
                   const int channels, int min_win_size, int max_window_size) {
...
void adaptive_median_filter_process(std::vector<unsigned char> *image, const int width, const int height,
                                    const int channels, int min_win_size, int max_window_size) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            temp_window.clear();
            current_win_size = min_win_size;
            temp_window = get_window(image, width, height, x, y, current_win_size);

            unsigned char pxl = (*image)[y * width + x];
            std::sort(temp_window.begin(), temp_window.end(), comp);
...
#pragma omp parallel for
    ...
    spdlog::info("Adaptive median filter completed in {} seconds", elapsed_seconds);
}
